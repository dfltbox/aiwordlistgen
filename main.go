...
func clean(resp string) ([]string, error) {
	cleanResp := strings.Trim(resp, "[]")
	cleanResp = strings.Trim(cleanResp, "\n")
	words := strings.Split(cleanResp, ",")
	//expression generated by chatgpt
	xp := regexp.MustCompile(`\[\s*((-?\d+|'[^']*'|"[^"]*")(,\s*(-?\d+|'[^']*'|"[^"]*"))*)?\s*\]`)
	if !xp.MatchString(resp) {
		return nil, fmt.Errorf("0")
	}
	var result []string
...
func generate() (string, error) {
	...
	words, err := clean(response.Response)
	if err != nil {
		return "", fmt.Errorf("0")
	}
...
func main() {
	...
	for i := 0; i < iterations; i++ {
		...
		if err == nil {
			...
			fmt.Println("LLM returned an invalid array, retrying!")
			i--
		}
